trigger Account_trigger_vod on Account bulk (before delete, after delete) {
    ...
    if (Trigger.isBefore) { 
        ...
        if (loserToMaster.size() > 0) {
            Set<ID> loserRecordIds = loserToMaster.keyset();
            Map<Id, EM_Attendee_vod__c> attendeesWithLoser = new Map<Id, EM_Attendee_vod__c>([Select Id, Walk_In_Reference_ID_vod__c
                                                                                 FROM EM_Attendee_vod__c
                                                                                 WHERE Walk_In_Reference_ID_vod__c in :loserRecordIds]);
            List<EM_Attendee_vod__c> attendeesToUpdate = new List<EM_Attendee_vod__c>();

            for (Id recordId : attendeesWithLoser.keySet()) {
                EM_Attendee_vod__c attendee = attendeesWithLoser.get(recordId);
                Id winningRecord = loserToMaster.get(attendee.Walk_In_Reference_ID_vod__c);

                attendeesToUpdate.add(new EM_Attendee_vod__c(Id = recordId,
                                                           Walk_In_Reference_ID_vod__c = winningRecord));
            }
