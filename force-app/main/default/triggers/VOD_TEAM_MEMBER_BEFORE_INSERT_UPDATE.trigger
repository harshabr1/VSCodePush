trigger VOD_TEAM_MEMBER_BEFORE_INSERT_UPDATE on EM_Event_Team_Member_vod__c (before insert, before update) {
    ...
    for (EM_Event_Team_Member_vod__c existingMember : existingTeamMembers) {
        for(EM_Event_Team_Member_vod__c newMember: Trigger.new) {
            if ((Trigger.isInsert || (Trigger.isUpdate && Trigger.oldMap.get(newMember.Id).Team_Member_vod__c != newMember.Team_Member_vod__c)) &&
            ...
            } else if((Trigger.isInsert || (Trigger.isUpdate && Trigger.oldMap.get(newMember.Id).Group_Name_vod__c != newMember.Group_Name_vod__c)) &&
